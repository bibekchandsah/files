...
// Display the BST in a graphical way
void display(struct Node *root)
{
    // Declare a variable to store the level of the node
    int level = 0;

    // Declare a helper function to print the spaces and slashes
    void printHelper(struct Node * node, int level)
    {
        // If the node is not NULL, recursively print the right subtree, then the node data, then the left subtree
        if (node != NULL)
        {
            // Increment the level by 1
            level++;

            // Print the right subtree with spaces and slashes
            printHelper(node->right, level);

            // Print the spaces according to the level
            for (int i = 0; i < level - 1; i++)
            {
                printf("   ");
            }

            // Print the node data with slashes
            printf("%d", node->data);
            ...
            // Print the left subtree with spaces and slashes
            printHelper(node->left, level);
        }
    }

    // Call the helper function with the root and level
